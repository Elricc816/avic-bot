...
module.exports = (client) => {
  ...
  client.distube.on('playSong', (queue, song) => {
    queue.textChannel?.send(`<:play:1533532009450831945> Now playing: **${song.name}** - \`${song.formattedDuration}\``);
  });

  client.distube.on('addSong', (queue, song) => {
    queue.textChannel?.send(`<:yes:1533858344341602365> Added **${song.name}** to the queue.`);
  });
